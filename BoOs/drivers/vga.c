...
#define VGA_MEMORY 0xA0000  // VGA memory address for mode 13h (320x200, 256 colors)
#define SCREEN_WIDTH 320
#define SCREEN_HEIGHT 200

// Clears the screen by setting all pixels to the specified color
void clearVGA(byte color) {
    unsigned char* screen = (unsigned char*)VGA_MEMORY;  // VGA memory starts at 0xA0000

    // Set every pixel to the specified color
    for (int i = 0; i < SCREEN_WIDTH * SCREEN_HEIGHT; ++i) {
        screen[i] = color;  // Set the pixel color in VGA memory
    }
}

// Sets a pixel at (x, y) to the specified color
void set_pixel(int x, int y, byte color) {
    unsigned char* screen = (unsigned char*)VGA_MEMORY;  // VGA memory pointer

    // Make sure the coordinates are within bounds
    if (x >= 0 && x < SCREEN_WIDTH && y >= 0 && y < SCREEN_HEIGHT) {
        // Calculate the position of the pixel in the VGA memory
        screen[y * SCREEN_WIDTH + x] = color;  // Set the pixel color
    }
}
void set_vga_color(int index, byte r, byte g, byte b) {
    outp(0x3c8, index);  // Set the palette index
    outp(0x3c9, r);      // Set the red component
    outp(0x3c9, g);      // Set the green component
    outp(0x3c9, b);      // Set the blue component
}
